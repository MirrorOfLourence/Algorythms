...
{
	Point v1(C.getX() - B.getX(), C.getY() - B.getY());		//точками задаём векторы: 1 из данных точек (BC)
		...
		while (true)
		{
			...
			else	//иначе сравниваем с новой прямой, составленной из двух предыдущих добавленных точек
			{
				for (int i = 0; i < length-numOperated; i++)
				{
					...
					if (i == length - numOperated - 1)
					{
						value = getAngle(v[length-numOperated], v[length-numOperated+1], v[length-1]);
						if (value > maxPr)
						{
