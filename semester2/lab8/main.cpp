...
int main()
{
	...
	n++;
	tagsOfVertexes.resize(n, INT_MAX);
	...
	{
		if (visitedVertexes[i] == false)	//если вершина ещё не посещена
		{
			int worked = 0;
			int limit = 0;
			...
			for (Edge edge : edges)	//ищем соседние данной вершины
			{
				if ((edge.vertex1 == i) || (edge.vertex2 == i))
				{
					int j = (edge.vertex1 == i) ? edge.vertex2 : edge.vertex1;
					if (!visitedVertexes[j])
					{
						int currentTag = tagsOfVertexes[i] + edge.weight;
						if(currentTag < tagsOfVertexes[j]) { tagsOfVertexes[j] = currentTag; }	//если текущая метка вершины меньше предыдущей, меняем
						worked++;
					}
				}
